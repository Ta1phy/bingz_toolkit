...
def get_user_data_dir():
    ...
    if os.name == 'nt':  # Windows
        ...
        return os.path.join(app_data, 'BingZtoolkit')
    elif os.name == 'posix':  # macOS或Linux
        home = os.path.expanduser('~')
        if sys.platform == 'darwin':  # macOS
            return os.path.join(home, 'Library', 'Application Support', 'BingZtoolkit')
        else:  # Linux
            return os.path.join(home, '.config', 'BingZtoolkit')
    # 默认返回当前目录
...
class UpdateDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # 标题
        title_label = QLabel("BingZ toolkit - 更新检查")
...
class AIToolManager(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle(f"BingZ toolkit v{self.current_version}")
